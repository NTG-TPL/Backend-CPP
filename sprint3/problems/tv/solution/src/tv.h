...
public:
    ...
    [[nodiscard]] std::optional<int> GetChannel() const noexcept {
        return is_turned_on_ ? std::optional{current_channel_} : std::nullopt;
    }
    ...
    void SelectLastViewedChannel() {
        ...
        std::swap(current_channel_, last_channel_);
    }

private:
    bool is_turned_on_ = false;
    int current_channel_ = 1, last_channel_ = current_channel_;
};