...
class Loot : private Object{
public:
using Id = util::Tagged<size_t, Loot>;
    Loot(LootType type, Point2d position, size_t loot_index):
            loot_type_(std::move(type)), Object(position), loot_index_(loot_index) {}

    [[nodiscard]] const LootType& GetLootType() const noexcept { return loot_type_;}
    [[nodiscard]] const Point2d& GetPosition() const noexcept {return position_;}
    [[nodiscard]] size_t GetTypeIndex() const noexcept {return loot_index_;}

private:
    const LootType loot_type_;
    const size_t loot_index_;
};
...
public:
    ...
    using Loots = std::unordered_map<Loot::Id, Loot, LootIdHasher>;
