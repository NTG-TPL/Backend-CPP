...
namespace http_handler {
    ...
    StringResponse ApiHandler::HandleApiRequest(const StringRequest& req){
        ...
        auto method_not_allowed = [&req](std::string_view text, std::string_view allow, std::string_view cache = std::string_view{}){
            return MakeTextResponse(req, http::status::method_not_allowed, text, std::string_view{}, allow);
        };
        ...
        if(decoded_target.starts_with(EndPoint::GAME)){
            ...
            if (decoded_target == EndPoint::JOIN) {
                ...
                return is_not_expected ? method_not_allowed(ErrorResponse::INVALID_POST, Api::POST, CacheControl::NO_CACHE) : RequestToJoin(req);
            }
        }

        return MakeTextResponse(req, http::status::method_not_allowed, ErrorResponse::BAD_REQ);
    }
...
/*
        ...
        try {
            ...
            obj = {{ApiKey::USER_TOKEN, *token},
                               {ApiKey::PLAYER_ID, *player.GetId()}};
        } catch (const std::exception &e) {
            return MakeTextResponse(req, http::status::internal_server_error, "exception", "Join Game Error :( call the fixies "s + e.what());
        }
    */
    ...
    StringResponse ApiHandler::RequestToMaps(const StringRequest& req, std::string & decoded_target){
        ...
        return MakeTextResponse(req, http::status::bad_request, ErrorResponse::BAD_REQ);
    }
}