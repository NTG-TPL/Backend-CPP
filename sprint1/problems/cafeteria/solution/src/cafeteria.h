...
public:
    Order(net::io_context& io, int id,
        ...
        : io_{ io }
        , id_{ id }
        , gas_cooker_{std::move(std::move( gas_cooker ))}
    ...
    void OnBreadBaked(sys::error_code ec) {
        ...
        if (ec) {
            logger_.LogMessage("Baking bread error : "s + ec.what(), bread_->GetId());
        }
        else {
            logger_.LogMessage("Bread has been baked."sv, bread_->GetId());
            try {
                bread_->StopBaking();
            }
            catch (std::exception& ex) {
