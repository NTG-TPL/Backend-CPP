...
namespace model {
    ...
    Building tag_invoke(value_to_tag<Building>, value const& json_value) {
        if (json_value.as_object().size() != 4) { throw std::logic_error(error_wrong_amount_keys + "Building"); }
    ...
    Road tag_invoke(value_to_tag<Road>, value const& json_value) {
        ...
        if (road.size() != 3) { throw std::logic_error(error_wrong_amount_keys + "Road"s); }
