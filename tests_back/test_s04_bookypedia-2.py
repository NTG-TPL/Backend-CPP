...
class Bookypedia:
    ...
    def edit_author(self, author_or_callback: Union[str, Callable], new_author: str) -> Optional[str]:
        ...
        print("IN edit_author FUNCTION!!!")
        print("request = ", request)
        print("chose = ", self.chose)
        print("db_name = ", self.db_name)
        print("process = ", self.process)
        print("new_author", new_author)
        if author_or_callback == self.empty_chooser:
            return request
        if request.startswith('Failed'):
            return request
        self.process.write(new_author)
        print("self._author = ", self._author)
        print("chose = ", self.chose)
        if self._wait_select():
            res =  self.process.read()
            print("process.read = ", read)
            return res
...
def test_edit_author_by_index(db_name):
    with run_bookypedia(db_name) as bookypedia:
        ...
        ass = bookypedia.edit_author(bookypedia.author_chooser(author), new_name)
        assert ass is None
        if db_name != 'empty_db':
            print('books2 = ', books)
            assert books == bookypedia.show_author_books(bookypedia.author_chooser(new_name))
